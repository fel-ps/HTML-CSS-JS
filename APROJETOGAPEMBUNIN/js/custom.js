jQuery(document).ready(function () {
	...
	* ===================================== **/
	if ($('.video-play-icon').length) {
		// $('.video-play-icon a').each(function () {
	...
	* ===================================== **/
	...
	$(function () {
		$('header.scroll a').click(function () {
			...
			$('html, body').animate({ scrollTop: $('#' + alvo).offset().top - 52 }, 1000);
			return false;
		});

		$('#load-more-event').click(function () {
			$('#agenda .upcoming-event').each(function (e) {
				console.log('ev', $(this))
				if ($(this).hasClass('hide')) {
					$(this).removeClass('hide').addClass('show').fadeIn(1000);
					$('#load-more-event').fadeOut(1000);
				}
			})
		})

		// na midia - change url nav
		// Obtém o caminho da URL atual
		var currentPath = window.location.pathname;

		// Define a rota desejada
		var desiredRoute = "/na-midia";

		// Verifica se a rota desejada está contida no caminho da URL
		if (currentPath.includes(desiredRoute)) {
			var currentURL = window.location.href; // Obtenha a URL completa
			var siteLink = currentURL.split('/na-midia')[0]; // Extraia a parte antes de "/na-midia"
			...
			$('header nav ul li').each(function () {
				...
				if (currentHref) {
					// Adicione o link do site dinâmico no início do atributo href
					link.attr('href', siteLink + currentHref);
				}
			});
		}
	});
